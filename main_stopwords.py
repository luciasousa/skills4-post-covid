...
for sentence in sentences_intro_pt:
    ...
    sentence = [word for word in sentence_tokens if not word in stopwords_pt]
    sentences_intro_pt.append(sentence)


sentences_intro_en = []
for sentence in sentences_intro_en:
    sentence_tokens = nltk.word_tokenize(str(sentence))
    sentence = [word for word in sentence_tokens if not word in stopwords_en]
    sentences_intro_en.append(sentence)
...
for sentence in sentences_proj_pt:
    sentence_tokens = nltk.word_tokenize(str(sentence))
    sentence = [word for word in sentence_tokens if not word in stopwords_pt]
    sentences_proj_pt.append(sentence)
...
for sentence in sentences_proj_en:
    sentence_tokens = nltk.word_tokenize(str(sentence))
    sentence = [word for word in sentence_tokens if not word in stopwords_en]
    sentences_proj_en.append(sentence)
'''

skills = []
sentence = ' '.join([re.sub(r'[^a-zA-Z\s]', '', str(s)) for s in sentences_intro_pt])
...
#print(skills)
with open("skills_engenharia_computacional_pt_sw/skills_introducao_pt_sw.txt", "w") as f:
    f.write('\n'.join(skills))

f.close()

skills = []
sentence = ' '.join([re.sub(r'[^a-zA-Z\s]', '', str(s)) for s in sentences_proj_pt])
sentence = ' '.join(dict.fromkeys(sentence.split()))
firstpart, secondpart = sentence[:len(sentence)//2], sentence[len(sentence)//2:]
print(sentence)
...
#print(skills)
with open("skills_engenharia_computacional_pt_sw/skills_projeto_pt_sw.txt", "w") as f:
    f.write('\n'.join(skills))
...
skills = []
sentence = ' '.join([re.sub(r'[^a-zA-Z\s]', '', str(s)) for s in sentences_intro_en])
sentence = ' '.join(dict.fromkeys(sentence.split()))
firstpart, secondpart = sentence[:len(sentence)//2], sentence[len(sentence)//2:]
...
#print(skills)   
with open("skills_engenharia_computacional_en_sw/skills_introducao_en_sw.txt", "w") as f:
    f.write('\n'.join(skills))

f.close()

skills = []
sentence = ' '.join([re.sub(r'[^a-zA-Z\s]', '', str(s)) for s in sentences_proj_en])
sentence = ' '.join(dict.fromkeys(sentence.split()))
firstpart, secondpart = sentence[:len(sentence)//2], sentence[len(sentence)//2:]
#compressed_data = zlib.compress(sentence.encode())
# Get the compressed data as a string
...
#print(skills)        
with open("skills_engenharia_computacional_en_sw/skills_projeto_en_sw.txt", "w") as f:
    f.write('\n'.join(skills))
